# PhpZabbixApi contribution

Thanks for your interest in PhpZabbixApi!
...
GitHub Issues is for **issues**, as opposed to question on how to use PhpZabbixApi.
If you are not sure this is a bug, or simply want to ask such a question,
please post your question on [Stack Overflow](http://stackoverflow.com/questions/tagged/phpzabbixapi),
using the `phpzabbixapi` tag.

If you happen to find a bug, we kindly request you report it. However,
before submitting it, please check the project documentation.
...
* If your issue is an error or exception, you MUST provide us with a stack trace. **Do not** make a screenshot of the stack trace, as screenshots are
...
> _NOTE:_ Don't hesitate to give as much information as you can (OS, Zabbix version, PHP
> version, extensions...)

## Pull Requests

All the PhpZabbixApi team will be glad to review your code changes propositions! :smile:
...
This project follows [PSR-1](http://www.php-fig.org/psr/psr-1/) and [PSR-2](http://www.php-fig.org/psr/psr-2/) for coding style,
...
The documentation is mostly written with the `markdown` format.
...
* The `@expectedException*` annotations are prohibited. Use `\PHPUnit\Framework\TestCase::setExpectedException()`.
 ...
 *
 * @deprecated since confirm-it-solutions/php-zabbix-api 42.x, to be removed in 43.0. Use New\Class instead.
 */
final class LegacyClass
{
}

final class CurrentApiClass
{
    /**
     * NEXT_MAJOR: Remove this property.
     *
     * @deprecated since confirm-it-solutions/php-zabbix-api 42.x, to be removed in 43.0.
     */
    ...
    public function doSomething()
    {
        @trigger_error(sprintf(
            'Method "%s()" is deprecated since confirm-it-solutions/php-zabbix-api 42.x and will be removed in version 43.0.',
            __METHOD__
        ), \E_USER_DEPRECATED);
...
if (/* some condition showing the user is using the legacy way */) {
    @trigger_error(
        'This is deprecated since confirm-it-solutions/php-zabbix-api 42.x and will not be supported in version 43.0.',
        \E_USER_DEPRECATED
    );
} else {
...
- Lower version dropping is accepted as a Backward Compatible change according to [semver](https://semver.org/),
but some extra rules MUST be respected here:
  - PHP versions that are under the [orange zone](https://www.php.net/supported-versions.php) (Security Support) **MUST NOT** be dropped on the stable branch.
  - PHP versions that are under the [green zone](https://www.php.net/supported-versions.php) (Active Support) **MUST NOT** be dropped on the master branch.
...
This fixes a bug that arises when doing this or that, because `baz()` has a
required argument that might not be defined.
...
```
Change method to return `string` instead of `int`

This is a consensus made on #4242 in addition to #1337.

We agreed that using `string` is more flexible and scalable.
```
